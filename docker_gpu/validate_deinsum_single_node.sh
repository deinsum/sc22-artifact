...
do
    echo "Running ${prog} GPU validation with ${num_nodes} MPI processes."
    LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libgomp.so.1 mpirun -n ${num_nodes} python3 /dace/samples/distributed/${prog}_validation_gpu.py
done
